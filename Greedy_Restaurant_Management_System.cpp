...
void main()
{
	...
	float x;
	char choice;
	printf("\n Enter revenue per item:");
	for(i=0;i<5;i++) scanf("%f",&itemrevenue[i]);
	printf("\n enter minimum target average revenue for consecutive three orders:");
	scanf("%f",&x);
	for(;;)
	{
		...
		switch(choice)
		{
			...
			{
				int tot_rev[50];
				for(int k8=0;k8<5;k8++) tot_rev[j]=0;
				for(int k7=0;k7<5;k7++) tot_rev[j]=tot_rev[j]+order[j].item[k7]*itemrevenue[k7];
				if((tot_rev[j]+tot_rev[j+1]+tot_rev[j+2])/3>=x)
				{
